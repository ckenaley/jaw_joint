...
Code and data for XXXX et al. (202X).
...
The data directory is supplied here as a compressed .zip file and must
by uncompressed.
...
This script reads force/displacement trials from passive jaw-joint
loading experiments, converts force and angular displacement data into
torque-angle curves, estimates angular stiffness from trial-level
regressions, generates manuscript plots, and runs statistical analyses
comparing actinopterygian mandibular symphyseal joints with the rat
temporomandibular joint reference. Two stiffness regions are estimated
from each trial: an initial low-angle interval from 0–5° and a mid-angle
interval from 5–20°. The script first analyzes stiffness values using
linear mixed-effects models with taxon, curve segment, and their
interaction as fixed effects and specimen identity as a random effect.
Follow-up Dunnett-adjusted contrasts compare each fish taxon with the
rat reference separately for the low- and mid-angle intervals.

The script also runs phylogenetic mixed models using `MCMCglmm` to
evaluate whether the comparison between actinopterygian mandibular
symphyseal joints and the rat TMJ reference is robust to phylogenetic
non-independence. These models use a time-scaled tree containing the
four fish species and the rat reference taxon. Joint system, coded as
mandibular symphyseal joint for fishes and temporomandibular joint for
the rat, is included as a fixed effect, with phylogenetic species
identity and specimen identity included as random effects. Separate
phylogenetic models are fit for the low- and mid-angle stiffness
intervals.
...
- `tt_sp.nwk`: A time-scaled Newick-format phylogeny containing the taxa
  included in the stiffness analysis: largemouth bass
  (`Micropterus_nigricans`), yellow perch (`Perca_flavescens`), white
  perch (`Morone_americana`), chain pickerel (`Esox_niger`), and the rat
  reference taxon (`Rattus_norvegicus`). This tree is used only for the
  phylogenetic mixed models. Tip labels must match the species names
  used internally by the script.

Any inquiries concerning data and the analysis can be made to XXXX
